...
const restaurant = {
  ...
  order(starterIndex, mainIndex) {
    return [this.starterMenu[starterIndex], this.mainMen[mainIndex]];
  },
...
// 🔴 Delayed Departue from FAQ to TXL (11h25)
//              Arrival from BRU to FAQ (11h45)
// 🔴 Delayed Arrival from HEL to FAQ (12h05)
//            Departue from FAQ to LIS (12h30)
...
for (const flight of flights.split("+")) {
  ...
  const output = `${type.startsWith("_Delayed") ? "🔴" : ""}${type.replaceAll(
    "_",
    " "
  )} ${getCode(from)} ${type.startsWith("_Delayed") ? "from" : "to"} ${getCode(to)} (${time.replace(":", "h")})`.padStart(36);
  console.log(output);
}
...
console.log(maskCreditCard(23423421));
console.log(maskCreditCard(4323863484723427));
console.log(maskCreditCard("76783471234792487129"));
...
if (user.length >= 0) console.log(users[0].name); else console.log("User array empty");
